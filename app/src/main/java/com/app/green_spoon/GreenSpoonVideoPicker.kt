...
public class GreenSpoonVideoPicker(var mActivity: AppCompatActivity,
                                   var isForcefullyGetPermission: Boolean,
                                   var code: Int,
                                   var mOnVideoPickListener: OnVideoPickListener,
    ...
    val STORAGE_PERMISSION = 4564

    val SETTING_CAMERA_PERMISSION = 8451
    val SETTING_GALLERY_PERMISSION = 4241

    val REQUEST_IMAGE_CAPTURE = 3511
    val REQUEST_GALLERY_IMAGE = 7854
    val ONLY_STORAGE_PERMISSION = 1254
...
//        mActivity.startActivity(mIntent)
    ...
    fun captureImageForCamera() {
        checkForPermission(CAMERA_STRING_PERMISSION, CAMERA_PERMISSION, this@GreenSpoonVideoPicker)
    }

    fun pickGalleryImage() {
        checkForPermission(WRITE_STORAGE_STRING_PERMISSION, ONLY_STORAGE_PERMISSION, this@GreenSpoonVideoPicker)
    }
...
//        photoPickerIntent.type = "image/*"
    ...
    public fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == REQUEST_IMAGE_CAPTURE) {
            if (resultCode == Activity.RESULT_OK) {
                if (mOnVideoPickListener != null) {
                    var mUri = FileProvider.getUriForFile(mActivity, mActivity.packageName + ".provider", mFileImage!!)
                    mOnVideoPickListener!!.onVideoCaptureSuccess(mUri)
                    mOnVideoPickListener!!.onVideoPickSuccess(mUri, mFileImage!!.absolutePath)
                }
            } else {
                if (mOnVideoPickListener != null) {
                    mOnVideoPickListener!!.onVideoPickFailed()
                    ...
                    if (data != null) {
                        ...
                        if (mGalleryImage != null) {
                            if (mOnVideoPickListener != null) {
                                ...
                                mOnVideoPickListener!!.onVideoPickSuccess(mGalleryImage, mPath)
                            }
                        }
                    }
                } catch (e: Exception) {
    ...
    private fun openImagePickerDialog() {
        ...
        txt_title.text = "Add Video!"
        txt_take_photo.text = "Capture Video"
